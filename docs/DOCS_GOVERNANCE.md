...
| 情報の種類 | 書き先 |
|---|---|
| Miner要件/完了条件変更 | `docs/MINER_SPEC.md` |
| 閾値/ガード変更 | `docs/OPERATION_POLICY.json` |
| Miner実行手順変更 | `docs/MINER_RUNBOOK.md` |
...
## 3. 更新トランザクション（必須手順）
### 3.1 要件・運用変更時
1. `docs/MINER_SPEC.md` を更新
2. `docs/OPERATION_POLICY.json` を同期
3. 実行手順に影響がある場合は `docs/MINER_RUNBOOK.md` を同期
