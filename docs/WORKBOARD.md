...
| WB-BK-005 | P1 | ロジック | A段階のbig word時差スケジューラ（新しい順前提で、経過時間に応じて許可ページ数を自動計算） | Phase C更新後にA/Cの整合仕様が固まった後 |
| WB-BK-006 | P1 | 運用 | A段階のHTML補完ログを継続収集し、big word別の「50件蓄積速度」を学習更新する | WB-BK-005の最小版が動作し、記録フォーマットが固定された後 |
| WB-BK-007 | P2 | 自動化 | 探索ボタン廃止に向けた0時自動探索（PR上限到達まで）への段階移行設計 | A/B/Cの受け入れ試験が連続安定し、手動介入頻度が十分低下した後 |
...
| 2026-02-24 | DEC-027 | Minerの正本を `MINER_SPEC.md`（要件+DoD+探索仕様）と `MINER_RUNBOOK.md`（実行手順）に統合し、旧6文書は `docs/archive/miner_legacy/` に退避する | 情報量を維持したまま散乱を減らし、参照先を最小化するため |
