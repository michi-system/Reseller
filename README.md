# Reseller

このリポジトリの引き継ぎ入口は次です。  
`/Users/tadamichikimura/Downloads/dev-HQ/ebayminer/docs/START_HERE.md`

## Naming
- Project (display): `Reseller`
- Tool 1 (display): `Miner` / internal: `ebayminer`
- Tool 2 (display): `Operator` / internal: `listing-ops` (planned)

## Documentation
- 全体索引: `/Users/tadamichikimura/Downloads/dev-HQ/ebayminer/docs/README.md`
- 2ツール全体像: `/Users/tadamichikimura/Downloads/dev-HQ/ebayminer/docs/PROGRAM_OVERVIEW.md`
- 運用手順 (Codex + GitHub): `/Users/tadamichikimura/Downloads/dev-HQ/ebayminer/docs/OPERATIONS_MANUAL.md`
- GitHub初期設定: `/Users/tadamichikimura/Downloads/dev-HQ/ebayminer/docs/GITHUB_PROJECT_SETUP.md`
- ローカルDB方針: `/Users/tadamichikimura/Downloads/dev-HQ/ebayminer/docs/LOCAL_DB_STRATEGY.md`
- internal名移行計画: `/Users/tadamichikimura/Downloads/dev-HQ/ebayminer/docs/INTERNAL_NAME_MIGRATION.md`
- データ契約: `/Users/tadamichikimura/Downloads/dev-HQ/ebayminer/docs/DATA_CONTRACT_APPROVED_LISTING.md`
- 記録先ルール: `/Users/tadamichikimura/Downloads/dev-HQ/ebayminer/docs/DOCS_GOVERNANCE.md`
- 工数/計画/実装管理: `/Users/tadamichikimura/Downloads/dev-HQ/ebayminer/docs/WORKBOARD.md`
- 生成物一覧: `/Users/tadamichikimura/Downloads/dev-HQ/ebayminer/docs/RECORDS_REGISTRY.md`
- 機械可読マニフェスト: `/Users/tadamichikimura/Downloads/dev-HQ/ebayminer/docs/HANDOVER_MANIFEST.json`
